...
def predict_future_links(k):
    ...
    edges_list2 = get_edge_driection(edges_list)
    list_tupels_of_edge_to_check = remove_train_edges(list_tupels_of_edge, edges_list, edges_list2)

    neighbors_prop = get_neighbors_prop(H, nodes)
    edges_popularity = get_edges_popularity(list_tupels_of_edge_to_check, df, nodes)
    neighborhood_overlap = get_m(list_tupels_of_edge_to_check, H, df, nodes,neighbors_prop)

    probability = get_probability_for_each_edges(list_tupels_of_edge_to_check, edges_popularity, neighborhood_overlap,0.7)
    future_links = dict(sorted(probability.iteritems(), key=operator.itemgetter(1), reverse=True)[:number_of_nodes_to_predict]).keys()
...
def get_neighborhood_overlap(list_tupels_of_edge,G , nodes , probabilities):
    edges_no_overlap = {}
    for edge_data in list_tupels_of_edge:
        x = edge_data[0]
        y = edge_data[1]
...
def get_neighbors_prop(G, nodes):
    probabilities = {}
    ...
    for i in range(0, len(nodes)):
        ...
        for j in range(0, len(nodes)):
            ...
            y = nodes[j]
            Ey = set(G.neighbors(y))
            neighbors[y] = len(Ex & Ey)
        probabilities[nodes[i]] = normalizing_dictionary_values(neighbors)
...
def get_m(list_tupels_of_edge,G ,df, nodes ,neighbors_prop):
    degrees = get_degree_for_all_nods(df, nodes)
    probabilities = normalizing_dictionary_values(degrees)
    ...
    for edge_data in list_tupels_of_edge:
        ...
        py = probabilities[y]
        if px > py:
            c = neighbors_prop[x][y]
        else:
            c = neighbors_prop[y][x]

        edges_no_overlap[(x,y)] = c
    return edges_no_overlap
...
def cross_validation():
    ...
    for yy in range(0, 5):
        ...
        for train_index, test_index in kf.split(edges_list):
            ...
            neighborhood_overlap = get_m(list_tupels_of_edge_to_check,H ,train_df, nodes ,neighbors_prop) #get_neighborhood_overlap(list_tupels_of_edge_to_check, H , nodes)
